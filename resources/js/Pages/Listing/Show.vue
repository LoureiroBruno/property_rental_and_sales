<template>
    <div class="flex flex-col-reverse md:grid md:grid-cols-12 gap-4">
        <Box v-if="listing && listing.images && listing.images.length" class="md:col-span-7 flex items-center">
            <div class="grid grid-cols-2 gap-1">
                <img v-for="image in listing.images" :key="image.id" :src="getImageUrl(image.src)" alt="Listing image"
                    class="w-full h-48 object-cover rounded" />
            </div>
        </Box>
        <EmptyState v-else class="md:col-span-7 flex items-center">No images12</EmptyState>
